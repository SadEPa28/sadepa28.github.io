...
const resolve = async (inputUrl, basedir, extensions) => {
  const options = {
...
const plugin = (options = {}) => {
  ...
  const resolve$1 = (_options$resolve = options.resolve) !== null && _options$resolve !== void 0 ? _options$resolve : resolve;
      ...
      for await (const {
        ...
        try {
          const {
            source,
            from
          } = await resolve$1(url, basedir, extensions);
          ...
          const imported = await postcss__default['default'](plugin(options)).process(source, { ...opts,
            from
          });
...
const plugin$1 = (options = {}) => {
  ...
  return {
    ...
    async Once(css, {
      ...
      const map = await mm((_css$source$input$map = css.source.input.map) === null || _css$source$input$map === void 0 ? void 0 : _css$source$input$map.text).resolve(path__default['default'].dirname(file)).toConsumer();
      const urlList = [];
...
const loader = {
  ...
  async process({
    ...
    if (options.import) plugins.push(plugin({
      extensions: options.extensions,
      ...options.import
    }));
    if (options.url) plugins.push(plugin$1({
    ...
    if (options.minimize) {
      const p = cssnano__default['default'](typeof options.minimize === "object" ? options.minimize : {});
      plugins.push(p);
    } // Avoid PostCSS warning


    if (plugins.length === 0) plugins.push(plugin$3);
...
const loader$2 = {
  ...
  async process({
    ...
    const useFibers = (_options$fibers = options.fibers) !== null && _options$fibers !== void 0 ? _options$fibers : type === "sass";
    const fiber = useFibers ? loadModule("fibers") : undefined;
    const sync = (_options$sync = options.sync) !== null && _options$sync !== void 0 ? _options$sync : type !== "node-sass" && !fiber;
...
class Loaders {
  constructor(options) {
    ...
    this.add(loader, loader$1, loader$2, loader$4, loader$3);
    ...
    async generateBundle(opts, bundle) {
      ...
      const getExtractedData = async (name, ids) => {
        ...
        return {
          name: fileName,
          css: res.css,
          map: mm(res.map).relative(path__default['default'].dirname(path__default['default'].resolve(dir, fileName))).toString()
        };
      };
      ...
      for await (const [name, ids] of emittedList) {
        ...
        if (loaderOpts.minimize) {
          ...
          const ps = [];
          const p = cssnano__default['default'](typeof loaderOpts.minimize === "object" ? loaderOpts.minimize : {});
          ps.push(p);
          const resMin = await postcss__default['default'](ps).process(res.css, {
            from: res.name,
            to: res.name,
            map: sourceMap && {
              inline: false,
