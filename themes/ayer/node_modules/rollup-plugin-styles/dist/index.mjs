    ...
    async Once(css, {
      ...
      for await (const {
        ...
        try {
          ...
          const imported = await postcss(plugin(options)).process(source, { ...opts,
            from
          });
...
const plugin$1 = (options = {}) => {
  ...
  return {
    ...
    async Once(css, {
      ...
      const map = await mm((_css$source$input$map = css.source.input.map) === null || _css$source$input$map === void 0 ? void 0 : _css$source$input$map.text).resolve(path.dirname(file)).toConsumer();
      const urlList = [];
    ...
    async generateBundle(opts, bundle) {
      ...
      const getExtractedData = async (name, ids) => {
        ...
        return {
          name: fileName,
          css: res.css,
          map: mm(res.map).relative(path.dirname(path.resolve(dir, fileName))).toString()
        };
      };
